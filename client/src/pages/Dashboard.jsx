...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-cream-50 pt-24 pb-12 px-4 relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute top-[0%] left-[0%] w-96 h-96 bg-emerald-900/5 rounded-full blur-[100px] -z-10"></div>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-serif font-bold text-emerald-950 mb-1">Assalamu Alaikum, {profile.name}</h1>
            <p className="text-slate-600 font-medium">Welcome to your Rohin Muslim Matrimony dashboard.</p>
          </div>
          {user.plan === 'free' && (
            <button onClick={() => navigate('/plans')} className="bg-gold-gradient text-emerald-950 px-6 py-2.5 rounded-full font-bold shadow-lg shadow-gold-500/20 hover:scale-105 transition-all">
              Upgrade Plan
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {/* Plan Status Card */}
          <div className="glass-card p-6 rounded-3xl shadow-sm border border-emerald-900/10 flex flex-col justify-between">
            <div>
              <h3 className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-3">Current Membership</h3>
              <div className="flex items-end gap-3 mb-2">
                <span className={`text-3xl font-serif font-bold capitalize ${user.plan === 'elite' ? 'text-gold-600' : user.plan === 'premium' ? 'text-emerald-600' : 'text-emerald-950'}`}>
                  {user.plan}
                </span>
              </div>
            ...
            {user.plan !== 'elite' && (
               <button onClick={() => navigate('/plans')} className="text-sm font-semibold text-emerald-800 hover:text-gold-600 transition-colors w-max">
          ...
          {/* View Limit Card */}
          <div className="glass-card p-6 rounded-3xl shadow-sm border border-emerald-900/10">
            ...
            {user.viewLimit < 9000 ? (
              <>
                <div className="w-full bg-slate-200 rounded-full h-2.5 mb-2 overflow-hidden shadow-inner">
                  <div 
                    className="bg-gradient-to-r from-emerald-600 to-gold-400 h-2.5 rounded-full" 
              ...
              <p className="text-sm font-medium text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg inline-block border border-emerald-200">
          ...
          <div className="glass-card p-6 rounded-3xl shadow-sm border border-emerald-900/10">
             ...
             <div className="flex flex-col gap-3">
               <div className="flex items-center gap-3 bg-white/60 p-3 rounded-xl border border-slate-100">
                 <div className={`w-8 h-8 rounded-full flex items-center justify-center ${profile.isPhotoPublic ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-200 text-slate-500'}`}>
               ...
               <button onClick={() => navigate('/edit-profile')} className="w-full py-2 border-2 border-emerald-900 text-emerald-950 rounded-xl text-sm font-bold hover:bg-emerald-50 transition-colors mt-1">
        ...
        {/* Call to Action Row */}
        <div className="bg-emerald-950 rounded-3xl p-8 md:p-12 shadow-xl border border-gold-500/20 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <button onClick={() => navigate('/search')} className="bg-gold-gradient text-emerald-950 px-10 py-4 rounded-full font-bold shadow-lg shadow-gold-500/20 hover:scale-105 transition-transform text-lg flex items-center justify-center gap-2 mx-auto">
